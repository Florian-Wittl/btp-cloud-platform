...
Web applications deployed with SAP Java Buildpack can run in an Apache TomEE 10 container, which is compatible with Apache Tomcat 10.1.x.
...
SAP Java Buildpack provides some default configurations for the Apache TomEE 10 application container. They can be customized by the application with the [Resource Configuration](resource-configuration-c893e9c.md) feature.
...
SAP Java Buildpack provides the default configurations for unlimited sessions for the Apache TomEE 10 application container. They can be customized by the application with the [Resource Configuration](resource-configuration-c893e9c.md) feature. To limit the number of active sessions, set the **`maxActiveSessions`** attribute of the `Manager` element in the application's `context.xml` file:
